<?php foreach ($companies as $company) { ?>
    <div id="{id_Entreprise}" class="bg-white flex rounded-md w-full justify-between overflow-hidden">
        <div class="flex flex-col h-full justify-between p-2">
            ...
            <div class="flex flex-wrap">
                <?php foreach ($company->tags as $nameTag) { ?>
                    <span class="bg-darkBlue text-white text-xs flex items-center w-fit py-1 px-3 ml-5 mt-2 leading-none rounded-xl"><?= $nameTag ?></span>
                <?php } ?>
            </div>
        </div>
